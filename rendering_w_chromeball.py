...
def create_chrome_material():
    """Create a chrome/metallic material"""
...
def create_objects_on_ground(ground):
    ...
    # Chrome ball
    bpy.ops.mesh.primitive_uv_sphere_add(radius=0.7, location=(2, 0, 0))
    chrome_ball = bpy.context.active_object
    chrome_ball.name = "ChromeBall"
    
    # Apply chrome material
    chrome_material = create_chrome_material()
    chrome_ball.data.materials.append(chrome_material)
    
    # Add subdivision surface modifier to make the chrome ball smoother
    subsurf = chrome_ball.modifiers.new(name="Subdivision", type='SUBSURF')
    subsurf.levels = 2
    subsurf.render_levels = 3
    
    # Snap objects to ground
    for obj in [cube, sphere, chrome_ball]:
    ...
    return cube, sphere, chrome_ball
...
def create_camera_with_tracking(cube, sphere, chrome_ball):
    # Calculate center between all objects
    cube_center = get_object_center(cube)
    sphere_center = get_object_center(sphere)
    chrome_center = get_object_center(chrome_ball)
    center_point = (cube_center + sphere_center + chrome_center) / 3
    ...
    camera_distance = 5  # Increased distance to fit all objects
    camera_height = 2.5  # Increased height for better angle
...
def main():
    ...
    cube, sphere, chrome_ball = create_objects_on_ground(ground)
    light = create_area_light(location=(0, -3, 3), energy=2000, size=2)
    camera, focus_empty = create_camera_with_tracking(cube, sphere, chrome_ball)
    
    # Create output directory
    output_dir = "/Users/yangmi/Documents/Projects/Blender_mesh/output_chromeball"
